...
- **胜负**：最终点数高者获胜，平局则打平
  ...
  ### 更新规则（v0.0.2）
 - **牌组设置**：使用6副牌进行游戏，可切换洗牌靴规则
   - 切换方法：修改 `lib/game_logic.dart` 中的 `const bool useShoe = false;` 为 `true` 即可启用洗牌靴规则
 - **洗牌靴规则**：剩余牌数低于25%～30%时自动重新洗牌
- **特殊玩法**：
  - **加倍（Double）**：前两张牌时可选择，下注金额加倍，只能再拿一张牌
  - **分牌（Split）**：前两张牌点数相同时可分牌，需下等额注，分成两手独立进行
  - **Blackjack（天然21点）**：A + 10点牌，赔率1:1.5
- **结算规则**：
  - 玩家Blackjack，庄家不是：玩家直接赢，赔率1.5倍
  - 双方都是Blackjack：和局
  - 庄家Blackjack，玩家不是：庄家直接赢
- **最低下注**：200 Y币，余额不足时无法开始游戏
- **天使投资系统**：余额不足时可借款5000 Y币，支持还款和借款记录
